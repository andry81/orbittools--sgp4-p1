 ...
 */
CoordTopocentric Observer::GetLookAngle(const Eci &eci) const
{
    /*
     * update the observers Eci to match the time of the Eci passed in
     * if necessary
     */
    Observer mutable_this = *this;

    mutable_this.Update(eci.GetDateTime());

    /*
     * calculate differences
     */
    Vector range_rate = eci.Velocity() - mutable_this.m_eci.Velocity();
    Vector range = eci.Position() - mutable_this.m_eci.Position();
